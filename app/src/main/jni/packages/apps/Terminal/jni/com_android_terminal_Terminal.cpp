 ...
 */
    ...
    static int term_damage(VTermRect rect, void *user) {
        ...
        return JVM::GetEnv()->CallIntMethod(term->getCallbacks(), damageMethod, rect.start_row,
                                            rect.end_row,
    ...
    static int term_moverect(VTermRect dest, VTermRect src, void *user) {
        ...
        return JVM::GetEnv()->CallIntMethod(term->getCallbacks(), moveRectMethod,
                                            dest.start_row, dest.end_row, dest.start_col,
                                            dest.end_col,
 ...
 */
    ...
    static jlong
    com_android_terminal_Terminal_nativeInit(JNIEnv *env, jclass clazz, jobject callbacks) {
    ...
    static jint com_android_terminal_Terminal_nativeResize(JNIEnv *env,
                                                           jclass clazz, jlong ptr, jint rows,
                                                           jint cols, jint scrollRows) {
        Terminal *term = reinterpret_cast<Terminal *>(ptr);
    ...
    static jint
    com_android_terminal_Terminal_nativeGetScrollRows(JNIEnv *env, jclass clazz, jlong ptr) {
        Terminal *term = reinterpret_cast<Terminal *>(ptr);
...
// https://gist.github.com/poutyface/1119105
    ...
    int register_com_android_terminal_Terminal() {
        ...
        ScopedLocalRef<jclass> localClass(env,
                                          env->FindClass(
                                                  (const char *) "app/simple/inure/decorations/terminal/TerminalCallbacks"));

        android::terminalCallbacksClass = reinterpret_cast<jclass>(env->NewGlobalRef(
                localClass.get()));

        android::damageMethod = env->GetMethodID(terminalCallbacksClass, "damage", "(IIII)I");
        android::moveRectMethod = env->GetMethodID(terminalCallbacksClass, "moveRect",
                                                   "(IIIIIIII)I");
        ...
        android::setTermPropIntMethod = env->GetMethodID(terminalCallbacksClass, "setTermPropInt",
                                                         "(II)I");
        android::setTermPropStringMethod = env->GetMethodID(terminalCallbacksClass,
                                                            "setTermPropString",
                                                            "(ILjava/lang/String;)I");
        android::setTermPropColorMethod = env->GetMethodID(terminalCallbacksClass,
                                                           "setTermPropColor",
                                                           "(IIII)I");
        ...
        ScopedLocalRef<jclass> cellRunLocal(env,
                                            env->FindClass(
                                                    (const char *) "app/simple/inure/decorations/terminal/Terminal$CellRun"));
