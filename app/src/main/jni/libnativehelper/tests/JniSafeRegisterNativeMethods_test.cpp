...
template<typename T>
auto stringify_helper(
        const T &val) -> decltype(std::stringstream().str()) {  // suppress incorrect warnings about compiler not support 'auto'
    std::stringstream ss;
    ss << val;
    return ss.str();
}
...
EXPECT_IS_VALID_JNI_ARGUMENT_TYPE(true,
                                  (is_valid_jni_argument_type<kNormalNative, /*pos*/0u, JNIEnv *>::value));
EXPECT_IS_VALID_JNI_ARGUMENT_TYPE(true,
                                  (is_valid_jni_argument_type<kNormalNative, /*pos*/1u, jobject>::value));
EXPECT_IS_VALID_JNI_ARGUMENT_TYPE(true,
                                  (is_valid_jni_argument_type<kNormalNative, /*pos*/1u, jclass>::value));
EXPECT_IS_VALID_JNI_ARGUMENT_TYPE(false,
                                  (is_valid_jni_argument_type<kNormalNative, /*pos*/1u, jstring>::value));
}
...
using namespace nativehelper::detail;
using traits_for_int_ret = FunctionTypeMetafunction<FN_ARGS_PAIR(
        test_function_traits::int_returning_function)>;
...
EXPECT_NO_MATCH_JNI_DESCRIPTOR_AGAINST_FUNCTION(kNormalNative, TestJni::v_eoo,
                                                "(Ljava/lang/Class;)V");

// OK.
EXPECT_MATCH_JNI_DESCRIPTOR_AGAINST_FUNCTION(kCriticalNative, TestJni::v_i, "(I)V");
EXPECT_MATCH_JNI_DESCRIPTOR_AGAINST_FUNCTION(kNormalNative, TestJni::v_eoo,
                                             "(Ljava/lang/Object;)V");
...
(void)
sTestCheckedAtFileScope;
(void)
sTestInferredAtFileScope;
...
CRITICAL_NATIVE_METHOD(TestJniMacros, v_lib,
"(JIZ)V");
OVERLOADED_CRITICAL_NATIVE_METHOD(TestJniMacros, v_lib,
"(JIZ)V", v_lib_od);
...
FAST_NATIVE_METHOD(TestJniMacros, v_eolib,
"(JIZ)V");
OVERLOADED_FAST_NATIVE_METHOD(TestJniMacros, v_eolib,
"(JIZ)V", v_eolib_od);
...
NATIVE_METHOD(TestJniMacros, v_eolib,
"(JIZ)V");
OVERLOADED_NATIVE_METHOD(TestJniMacros, v_eolib,
"(JIZ)V", v_eolib_od);
NATIVE_METHOD_AUTOSIG(TestJniMacros, v_eolib
);

_NATIVEHELPER_JNI_MAKE_METHOD_OLD(kNormalNative,
"v_eolib", "(JIZ)V", TestJniMacros_v_eolib);
tmp_native_method =
