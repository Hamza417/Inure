...
static int
on_csi(const char *leader, const long args[], int argcount, const char *intermed, char command,
       void *user) {
    VTermState *state = user;
    int leader_byte = 0;
    int intermed_byte = 0;
...
#define INTERMED(i, b) ((i << 16) | b)
    ...
    switch (intermed_byte << 16 | leader_byte << 8 | command) {
...

            {
                ...
                switch (val) {
                    ...
                    case 6: // CPR - cursor position report
                        vterm_push_output_sprintf_ctrl(state->vt, C1_CSI, "%s%d;%dR", qmark,
                                                       state->pos.row + 1, state->pos.col + 1);
                        break;
                }
            }
            break;
        ...
        case 0x72: // DECSTBM - DEC custom
            ...
            else UBOUND(state->scrollregion_bottom, state->rows);
        ...
        case 0x73: // DECSLRM - DEC custom
            ...
            else UBOUND(state->scrollregion_right, state->cols);
    ...
    if (cmdlen == 1)
        switch (command[0]) {
            ...
            case 'r': // Query DECSTBM
                vterm_push_output_sprintf_dcs(state->vt, "1$r%d;%dr", state->scrollregion_top + 1,
                                              SCROLLREGION_BOTTOM(state));
                return;
            case 's': // Query DECSLRM
                vterm_push_output_sprintf_dcs(state->vt, "1$r%d;%ds", SCROLLREGION_LEFT(state) + 1,
...
VTermState *vterm_obtain_state(VTerm *vt) {
    ...
    state->combine_chars = vterm_allocator_malloc(state->vt, state->combine_chars_size *
                                                             sizeof(state->combine_chars[0]));
