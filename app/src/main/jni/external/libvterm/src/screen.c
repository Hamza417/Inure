...
static ScreenCell *
realloc_buffer(VTermScreen *screen, ScreenCell *buffer, int new_rows, int new_cols) {
    ScreenCell *new_buffer = (ScreenCell *) vterm_allocator_malloc(screen->vt,
                                                                   sizeof(ScreenCell) * new_rows *
                                                                   new_cols);
...
static int putglyph(VTermGlyphInfo *info, VTermPos pos, void *user) {
    VTermScreen *screen = (struct VTermScreen *) user;
...
static int resize(int new_rows, int new_cols, VTermPos *delta, void *user) {
    ...
    screen->sb_buffer = (VTermScreenCell *) vterm_allocator_malloc(screen->vt,
    ...
    if (new_rows > old_rows) {
        if (!is_altscreen && screen->callbacks && screen->callbacks->sb_popline) {
            ...
            while (rows) {
                ...
                VTermPos pos = {0, 0};
                for (pos.col = 0;
                     pos.col < screen->cols; pos.col += screen->sb_buffer[pos.col].width)
...
static VTermScreen *screen_new(VTerm *vt) {
    ...
    VTermScreen *screen = (struct VTermScreen *) vterm_allocator_malloc(vt, sizeof(VTermScreen));
    int rows, cols;
...
static size_t _get_chars(const VTermScreen *screen, const int utf8, void *buffer, size_t len,
...
size_t
vterm_screen_get_text(const VTermScreen *screen, char *str, size_t len, const VTermRect rect) {
