    ...
    public void migrate(@NonNull SupportSQLiteDatabase supportSQLiteDatabase) {
        try {
            ...
            try {
                ...
                supportSQLiteDatabase.execSQL("CREATE TABLE foss_new (package_name TEXT NOT NULL, license TEXT NOT NULL, is_foss INTEGER NOT NULL, PRIMARY KEY(package_name))");
                supportSQLiteDatabase.execSQL("INSERT INTO foss_new (package_name, license, is_foss) SELECT package_name, '', is_foss FROM foss");
                supportSQLiteDatabase.execSQL("DROP TABLE foss");
                supportSQLiteDatabase.execSQL("ALTER TABLE foss_new RENAME TO foss");
            } catch (SQLiteException ignored) {
                try {
                    // It crashed again, so we'll just drop the table and recreate it
                    supportSQLiteDatabase.execSQL("DROP TABLE foss");
                    supportSQLiteDatabase.execSQL("CREATE TABLE foss (package_name TEXT NOT NULL, license TEXT NOT NULL, is_foss INTEGER NOT NULL, PRIMARY KEY(package_name))");
                } catch (SQLiteException ignored1) {
                }
            }
        }
    }
