...
open class ScopedAudioPlayerDialogFragment : DialogFragment(), SharedPreferences.OnSharedPreferenceChangeListener {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        if (StatusBarHeight.isLandscape(requireContext())) {
            window.attributes.height = FrameLayout.LayoutParams.MATCH_PARENT
        } else {
        ...
        if (StatusBarHeight.isLandscape(requireContext())) {
            ...
            window.attributes.height = (displayMetrics.heightPixels * 1f / 100f * 85f).toInt()
        } else {
            window.attributes.width = (displayMetrics.widthPixels * 1f / 100f * 90f).toInt()
        }
    }
