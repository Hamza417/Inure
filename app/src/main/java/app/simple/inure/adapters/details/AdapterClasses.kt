...
class AdapterClasses(private val list: ClassesNamesList, private val keyword: String) : RecyclerView.Adapter<AdapterClasses.Holder>() {
    ...
    override fun onBindViewHolder(holder: Holder, position: Int) {
        holder.xml.text = list.classNames[position]

        holder.xml.setOnClickListener {
            resourceCallbacks.onResourceClicked(list.classNames[position])
        }

        holder.xml.setOnLongClickListener {
            resourceCallbacks.onResourceLongClicked(list.classNames[position])
            true
        }
