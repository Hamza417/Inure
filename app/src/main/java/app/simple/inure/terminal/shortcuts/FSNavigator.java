...
public class FSNavigator extends android.app.Activity {
    ...
    private final android.content.Context context = this;
    ...
    private ImageView imageViewFolder(boolean up) {
        ...
        b1.setImageResource(up ? R.drawable.ic_folder_up : R.drawable.ic_snippet_folder);
    ...
    private LinearLayout directoryEntry(final String name) {
        ...
        if (up) {
            linearLayout = directoryView(up);
        } else {
            if (countDirectoryView < cachedDirectoryView.size()) {
                linearLayout = cachedDirectoryView.get(countDirectoryView);
            } else {
                linearLayout = directoryView(up);
                cachedDirectoryView.put(countDirectoryView, linearLayout);
            }
            ++countDirectoryView;
        }
        
        TextView tv = linearLayout.findViewById(R.id.textview);
        tv.setTag(name);
        tv.setText(up ? "[" + cd.getPath() + "]"
                        : name
                  );
        linearLayout.findViewById(R.id.imageview).setTag(name);
        return (linearLayout);
    }
    ...
    private void makeView() {
        ...
        LinearLayout linearLayout = scrollView.findViewById(R.id.mainview);
        ...
        if (zd != null) {
            ...
            for (int i = 0, n = zd.length; i < n; i++) {
                ...
                linearLayout.addView(directoryEntry(zd[i]));
        ...
        if (zf != null) {
            ...
            for (int i = 0, n = zf.length; i < n; i++) {
                linearLayout.addView(fileEntry(zf[i]));
                linearLayout.addView(entryDividerH());
            }
        }
